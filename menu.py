...
class Menu:
    ...
    '''So I have finished the menu. When I go to run it I run it through interfaces because if I run it through 
    the menu.py nothing shows up. The only commands that seem to work are 'listclasses' 'showclass' 'showrelationships' 'help' and 'exit'
    when I press the other commands it says invalid command. This means I have not yet implemented the other methods 
    that add a method, param, class, delete, rename,  and etc. The only commands that work right now are the interface methods
    
    I am finishing this at 1am but all that needds to be done is for the other classe to be impemented into the code'''
